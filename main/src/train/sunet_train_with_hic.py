# -*- coding: utf-8 -*-
"""
SUNet Training with HIC Tracking


Usage:
    python sunet_train_with_hic.py --loss L1 --epochs 500 --batch-size 4
    python sunet_train_with_hic.py --loss L2 --resume
    python sunet_train_with_hic.py --help
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='SUNet Training with HIC tracking')
...
paths = load_paths()
cfg_path = paths['configs_dir'] / f'training_SUNet_{args.loss}.yaml'
...
## Resume 
if Train['RESUME']:
    # Look for the most recent checkpoint file based on modification time
    import glob
    checkpoint_pattern = os.path.join(model_dir, 'model_latest_ep-*_bs-%d_ps-%d.pth'%(OPT['BATCH'], SUNet['PATCH_SIZE']))
    ...
    path_chk_rest = max(checkpoint_files, key=os.path.getmtime)
    print(f"Found {len(checkpoint_files)} checkpoint files, loading most recent: {path_chk_rest}")
...
def compute_hallucination_indices(model, epoch):
    ...
    try:
        ...
        try:
            ...
            print(f"Error in BaseHallucination: {e}")
            print(f"Traceback: {traceback.format_exc()}")
            raise
        if not hasattr(base_result, '__len__') or len(base_result) != 3:
            raise ValueError(f"BaseHallucination returned unexpected result: {base_result}")
        ...
        for wavelet in wavelets:
            ...
            if not hasattr(wavelet_result, '__len__') or len(wavelet_result) != 3:
                raise ValueError(f"Wavelet_Hallucination ({wavelet}) returned unexpected result: {wavelet_result}")
            
            wavelet_mse, wavelet_rmean_array, wavelet_rstd = wavelet_result
            
            # Extract scalar values
            wavelet_rmeans[wavelet] = float(np.mean(wavelet_rmean_array))
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    if epoch % Train['VAL_AFTER_EVERY'] == 0:
        ...
        if psnr_val_rgb > best_psnr:
            ...
            torch.save({'epoch': epoch,
                        ...
                        }, os.path.join(model_dir, "model_bestPSNR_ep-{}_bs-{}_ps-{}.pth".format(epoch, OPT['BATCH'], SUNet['PATCH_SIZE']))) 
    ...
    if epoch == 1 or epoch % HIC_EVAL_EVERY == 0:
        ...
        try:
            ...
            writer.add_scalar('rmean/base_rmean', base_rmean, epoch)
            writer.add_scalar('rmean/haar_rmean', haar_rmean, epoch)
            writer.add_scalar('rmean/db4_rmean', db4_rmean, epoch)
            writer.add_scalar('rmean/db8_rmean', db8_rmean, epoch)
    ...
    # Save the last model (delete previous checkpoint first to save space)
    new_checkpoint_path = os.path.join(model_dir, "model_latest_ep-{}_bs-{}_ps-{}.pth".format(epoch, OPT['BATCH'], SUNet['PATCH_SIZE']))
    
    # Delete previous checkpoint if it exists
    if epoch > start_epoch:
        prev_checkpoint_path = os.path.join(model_dir, "model_latest_ep-{}_bs-{}_ps-{}.pth".format(epoch-1, OPT['BATCH'], SUNet['PATCH_SIZE']))
