# -*- coding: utf-8 -*-
"""
LearnLet Training with HIC Tracking



Usage:
    python learnlet_train_with_hic.py --loss L1 --epochs 500 --batch-size 4
    python learnlet_train_with_hic.py --loss L2 --resume
    python learnlet_train_with_hic.py --help
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='LearnLet Training with HIC tracking')
...
paths = load_paths()
cfg_path = paths['configs_dir'] / f'training_LeLet_{args.loss}.yaml'
...
model_restored = Learnlet(n_scales=5, kernel_size=5, filters=64, exact_rec=True, thresh='hard')
...
def compute_hallucination_indices(model, epoch):
    ...
    try:
        ...
        if not hasattr(base_result, '__len__') or len(base_result) != 3:
            raise ValueError(f"Base hallucination function returned unexpected format: {type(base_result)} with length {len(base_result) if hasattr(base_result, '__len__') else 'N/A'}. Expected 4-tuple (MSEerror, RMean, RStd)")
        
        # Base hallucination function returns (MSEerror, RMean, RStd)
        ...
        for wavelet in wavelets:
            ...
            if not hasattr(wavelet_result, '__len__') or len(wavelet_result) != 4:
                raise ValueError(f"{wavelet} wavelet hallucination function returned unexpected format: {type(wavelet_result)} with length {len(wavelet_result) if hasattr(wavelet_result, '__len__') else 'N/A'}. Expected 4-tuple (MSEerror, HC, RMean, RStd)")
            
            wv_mse, wv_hc, wv_rmean_array, wv_rstd = wavelet_result
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    if epoch == 1 or epoch % HIC_EVAL_EVERY == 0:
        ...
        try:
            base_hic, haar_hic, db4_hic, db8_hic, base_rmean, haar_rmean, db4_rmean, db8_rmean = compute_hallucination_indices(model_restored, epoch)
